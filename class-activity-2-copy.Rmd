...
letters <- c("F", "D", "C", "B", "A")
df$student_scoreletter2<- cut(df$student_score,breaks = c(100,89,79,69,59,30), labels =letters)
```
...
```{r}
NUMBER = 1:25
df$logins = sample(NUMBER,100, replace = TRUE)
```
...
plot(df$logins, df$student_score, main = "Logins vs Scores", col=colors,pch=19)
```
...
DF<- iris
plot(DF)
#there is lineral relationship between Petal.Length and Petal. Width. Sepal.Length and Petal.Length.
```
```{r}
cor.test(DF$Petal.Length, DF$Petal.Width)
```
```{r}
